...
public class Linearizer {
    ...
    private Map<String, Map<Integer, Set<Production>>> lProd;
    ...
    public Linearizer(PGF pgf, String concrete)
        ...
        this(pgf, pgf.getConcrete(concrete));
    }
     ...
     * a given language
     **/
    private Map<String, Map<Integer, Set<Production>>>
    getLProductions()
            throws LinearizerException {
        Map<Integer, Set<Production>> emptyMap = new HashMap<Integer, Set<Production>>();
     ...
     **/
    private Map<String, Map<Integer, Set<Production>>>
    linIndex(Map<Integer, Set<Production>> productions) {
        Map<String, Map<Integer, Set<Production>>> vtemp =
                new HashMap<String, Map<Integer, Set<Production>>>();
        Iterator<Entry<Integer, Set<Production>>> i =
                productions.entrySet().iterator();
        while (i.hasNext()) {
            Entry<Integer, Set<Production>> entr =
                    (Entry<Integer, Set<Production>>) i.next();
            ...
            while (prods.hasNext()) {
                ...
                if (vs != null)
                    for (int j = 0; j < vs.size(); j++) {
                        HashMap<Integer, Set<Production>> htemp = new HashMap<Integer, Set<Production>>();
                        Set<Production> hSingleton = new HashSet<Production>();
                        hSingleton.add(prod);
                        String newl = vs.elementAt(j);
                        htemp.put(res, hSingleton);
                        if (vtemp.containsKey(newl)) {
                            Map<Integer, Set<Production>> obj = vtemp.get(newl);
                            if (obj.containsKey(res)) {
                                Set<Production> ttemp = obj.get(res);
                                ttemp.add(prod);
                                obj.remove(res);
                                obj.put(res, ttemp);
                                vtemp.remove(newl);
    ...
    getFunctions(Production p,
                 Map<Integer, Set<Production>> productions) {
        Vector<String> rez = new Vector<String>();
        ...
        else // coercion
        {
            ...
            Set<Production> prods = productions.get(fid);
            if (prods == null) return null;
            else {
                Iterator<Production> prod = prods.iterator();
                while (prod.hasNext()) {
     ...
     **/
    boolean conditionProd(int i, Map<Integer, Set<Production>> prods) {
     ...
     **/
    private Set<Production> filterProdSet1(Map<Integer, Set<Production>> prods0, Set<Production> set) {
        Set<Production> set1 = new HashSet<Production>();
     ...
     **/
    private Map<Integer, Set<Production>>
    filterProductions(Map<Integer, Set<Production>> prods0,
                      Map<Integer, Set<Production>> prods) {

        HashMap<Integer, Set<Production>> tempRez = new HashMap<Integer, Set<Production>>();
        boolean are_diff = false;
        for (int index : prods.keySet()) {
            Set<Production> setProd = prods.get(index);
            Set<Production> intermRez = filterProdSet1(prods0, setProd);
            if (!intermRez.isEmpty())
                tempRez.put(index, intermRez);
        }
        Iterator<Entry<Integer, Set<Production>>> it = tempRez.entrySet().iterator();

        HashMap<Integer, Set<Production>> prods1 = new HashMap<Integer, Set<Production>>(prods0);

        while (it.hasNext()) {
            Entry<Integer, Set<Production>> entr = it.next();
            Integer index = entr.getKey();
            Set<Production> hp = entr.getValue();
     ...
     **/
    private boolean filterRule(Map<Integer, Set<Production>> prods, Production p) {
        if (p instanceof ApplProduction) {
    ...
    private Vector<LinTriple> lin0(Vector<String> xs,
        ...
        if (tree instanceof Lambda) {
            ...
            if (tree instanceof Application)
                ...
                } while (tree instanceof Application);
    ...
    private Vector<AppResult> getApps(Map<Integer, Set<Production>> prods, CncType mb_cty, String f) throws LinearizerException {
        if (mb_cty == null)
            if (f.equals("_V") || f.equals("_B")) return new Vector<AppResult>();
                ...
                Iterator<Entry<Integer, Set<Production>>> it = prods.entrySet().iterator();
                while (it.hasNext()) {
                    Entry<Integer, Set<Production>> en = it.next();
            ...
            Set<Production> setProd = prods.get(fid);
            Vector<AppResult> rez = new Vector<AppResult>();
    ...
    private Vector<AppResult> toApp(CncType cty, Production p, String f, Map<Integer, Set<Production>> prods) throws LinearizerException {
        Vector<AppResult> rez = new Vector<AppResult>();
        if (p instanceof ApplProduction) {
            int[] args = ((ApplProduction) p).getArgs();
            ...
            int fid = ((CoerceProduction) p).getInitId();
            Set<Production> setProds = prods.get(fid);
